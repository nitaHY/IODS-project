...
g2 = g2 + geom_boxplot() + ggtitle("Student absences by alcohol consumption and sex")
g2
...
m2 <- glm(high_use ~ failures + absences + guardian, data = alc, family = "binomial")
...
cbind(OR, CI)
#if the value 1 was included in the exponentiated CI, then the p-value would have been > 0.05, meaning non-significance of the relationship
#for non-exponentiated values this value would have been 0
...
# average number of something?
loss_func(class = alc$high_use[1:5], prob = alc$probability[1:5])
head(alc$high_use)
head(alc$probability)
abs(alc$high_use[1:5] - alc$probability[1:5])
abs(alc$high_use[1:5] - alc$probability)[1:5] > 0.5
mean(abs(alc$high_use[1:5] - alc$probability)[1:5] > 0.5)
```
...
loss_func(class = alc$high_use, prob = probabilities) 
#0.2297297
...
cv10$delta[1]
#0.2378378
#prediction error may increase due to "unseen" data, so the prediction is not based on the same data the model was trained, so we can expect a little higher prediction error to test data than training data
